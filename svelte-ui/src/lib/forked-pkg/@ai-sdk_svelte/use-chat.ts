...
import { useSWR } from '../sswr.svelte';
...
const getStreamedResponse = async (
	...
	const constructedMessagesPayload = sendExtraMessageFields
		...
		: chatRequest.messages.map(({ role, content, data, annotations, toolInvocations }) => ({
				role,
				content,
				...(data !== undefined && { data }),
...
export function useChat({
	...
	const reload: UseChatHelpers['reload'] = async ({
		...
		return triggerRequest({
			messages:
				lastMessage?.role === 'assistant' ? messagesSnapshot.slice(0, -1) : messagesSnapshot,
			headers,
			body,
			data
		});
	...
	const setData = (
		dataArg: JSONValue[] | undefined | ((data: JSONValue[] | undefined) => JSONValue[] | undefined)
	) => {
